...
client = AsyncIOMotorClient(
    mongo_url,
    # Tuned for serverless (Vercel): fail fast instead of hanging up to the
    # driver's 30s default, and keep the pool small since each cold start
...
class BrandingImages(BaseModel):
    ...
    yogaIndividuel: str = ""
    pilatesIndividuel: str = ""
    pilatesTapis: str = ""
    pilatesTapisIndividuel: str = ""
    pilatesGa: str = ""
    pilatesGaIndividuel: str = ""
    logoUrl: str = ""
    faviconUrl: str = ""
    appIconUrl: str = ""
...
def _pick_best_forfait(candidates: list, category: Optional[str], class_kind: Optional[str]):
    """Among forfaits that already satisfy the match query, prefer the most
    specific one: a forfait whose category AND kind exactly match the class
    beats one that is more generic (category=None and/or kind=None), so a
    dedicated "Individuel" or category-specific pack is consumed before a
    catch-all one, even if the catch-all pack is older. Ties are broken by
    creation date (oldest first, list is already sorted that way)."""
    if not candidates:
        return None

    def specificity(f: dict) -> int:
        score = 0
        if f.get("category") is not None and f.get("category") == category:
            score += 1
        if f.get("kind") is not None and f.get("kind") == class_kind:
            score += 1
        return score

    best_score = max(specificity(f) for f in candidates)
    for f in candidates:
        if specificity(f) == best_score:
            return f
    return candidates[0]
...
async def mark_attendance(booking_id: str, user: dict = Depends(require_owner)):
    ...
    candidates = await db.forfaits.find(match_criteria, {"_id": 0}).sort("created_at", 1).to_list(200)
    matching = _pick_best_forfait(candidates, category, class_kind)
...
async def _apply_forfaits_to_uncovered(user_id: str) -> int:
    ...
    for b in bookings:
        ...
        candidates = await db.forfaits.find(criteria, {"_id": 0}).sort("created_at", 1).to_list(200)
        matching = _pick_best_forfait(candidates, category, class_kind)
        if not matching:
            continue
        history_entry = {
