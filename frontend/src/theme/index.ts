...
const defaultTexts = {
  ...
  calendarFilterPilatesTapis: "Pilates Tapis",
  calendarFilterPilatesGa: "Pilates G.A.",
  calendarFilterPilatesCollectif: "Pilates Collectif",
  calendarFilterYogaCollectif: "Yoga Collectif",
  calendarFilterYogaIndividuel: "Yoga Individuel",
  calendarFilterMassage: "Massages",
...
export async function refreshThemeFromBackend(backendUrl?: string): Promise<boolean> {
  ...
  try {
    ...
    // Network error (backend unreachable, cold start, CORS, etc.): keep
    // using cached/default values. Returning false here matters — the
    // caller must NOT treat this as success, otherwise a first-time visitor
    // whose very first request fails would reload forever without ever
    // actually caching anything (each reload looks like a fresh "first
    // visit" again since nothing was ever saved).
    return false;
  }
}
