...
const CATEGORIES_ALL = [
  ...
  { key: "pilates_tapis", label: "Pilates Tapis" },
  { key: "pilates_ga", label: "Pilates G.A." },
...
function categoriesForKind(kind: string) {
  if (kind === "group") return CATEGORIES_GROUP;
  if (kind === "private") return CATEGORIES_PRIVATE;
  return CATEGORIES_ALL; // "Tous" kind: any category can still apply
}
                  ...
                  {KINDS.map((k) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          const nextCats = categoriesForKind(k.key).map((c) => c.key);
                          setForm({
                            ...form,
                            kind: k.key,
                            category: nextCats.includes(form.category) ? form.category : "",
                          });
                        }}
                        style={[
                          styles.chip,
