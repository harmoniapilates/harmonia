...
if (Platform.OS === "web" && typeof window !== "undefined" && typeof document !== "undefined") {
  try {
    ...
    // Warm-up ping: hits the backend once on load in case a cold start is needed.
    ...
    // Safety net: even if something above is ever wrong again, never allow
    // more than one auto-reload attempt per browser tab session.
    const alreadyTriedThisSession =
      typeof window !== "undefined" &&
      window.sessionStorage?.getItem("harmonia_theme_reload_attempted") === "1";
    if (isFirstEverVisit && !alreadyTriedThisSession) {
      window.sessionStorage?.setItem("harmonia_theme_reload_attempted", "1");
