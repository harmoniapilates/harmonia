...
const CATEGORIES_GROUP = ["pilates", "yoga"];
const CATEGORIES_PRIVATE = ["pilates_tapis", "pilates_ga", "yoga", "massage"];
const CATEGORY_LABELS: Record<string, string> = {
  yoga: "Yoga",
  pilates: "Pilates",
  pilates_tapis: "Pilates Tapis",
  pilates_ga: "Pilates G.A.",
  massage: "Massages",
};
function categoriesForKind(kind: string) {
  return kind === "private" ? CATEGORIES_PRIVATE : CATEGORIES_GROUP;
}
                ...
                <View style={styles.cardTop}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.cardMeta}>
                      {CATEGORY_LABELS[cls.category] || cls.category} · {cls.kind === "private" ? "Individuel" : "Groupe"}
                    </Text>
          ...
          >
            <View style={styles.modal}>
              ...
              <ScrollView keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.chipsRow}>
                  {["group", "private"].map((k) => (
                    <TouchableOpacity
                      ...
                      testID={`form-kind-${k}`}
                      onPress={() => {
                        const nextCats = categoriesForKind(k);
                        setForm({
                          ...form,
                          kind: k,
                          category: nextCats.includes(form.category) ? form.category : nextCats[0],
                        });
                      }}
                      style={[
                        styles.chip,
                ...
                <View style={styles.chipsRow}>
                  {categoriesForKind(form.kind).map((c) => (
                    <TouchableOpacity
                      key={c}
          ...
          >
            <View style={styles.modal}>
              ...
              <ScrollView keyboardShouldPersistTaps="handled">
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setBulkForm({
                          ...bulkForm,
                          kind: k,
                          category: nextCats.includes(bulkForm.category) ? bulkForm.category : nextCats[0],
                        });
                      }}
                      style={[
                        styles.chip,
                ...
                <Text style={styles.formLabel}>Catégorie</Text>
                <View style={styles.chipsRow}>
                  {categoriesForKind(bulkForm.kind).map((c) => (
                    <TouchableOpacity
                      key={c}
