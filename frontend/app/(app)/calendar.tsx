...
const CATEGORIES_KEYS = [
  ...
  { key: "pilates_collectif", labelKey: "calendarFilterPilatesCollectif" as const, category: "pilates", kind: "group" },
  { key: "yoga_collectif", labelKey: "calendarFilterYogaCollectif" as const, category: "yoga", kind: "group" },
  { key: "pilates_tapis", labelKey: "calendarFilterPilatesTapis" as const, category: "pilates_tapis" },
  { key: "pilates_ga", labelKey: "calendarFilterPilatesGa" as const, category: "pilates_ga" },
  { key: "yoga_individuel", labelKey: "calendarFilterYogaIndividuel" as const, category: "yoga", kind: "private" },
  { key: "massage", labelKey: "calendarFilterMassage" as const, category: "massage" },
];
...
function categoryImage(cat: string, kind?: string) {
  const priv = kind === "private";
  if (cat === "yoga") return (priv && images.yogaIndividuel) || images.yoga;
  if (cat === "pilates_tapis") return images.pilatesTapisIndividuel || images.pilates;
  if (cat === "pilates_ga") return images.pilatesGaIndividuel || images.pilates;
  if (cat === "pilates") return images.pilates; // group Pilates, or legacy private Pilates
...
export default function Calendar() {
  ...
  const filtered = useMemo(() => {
    ...
    const entry = CATEGORIES_KEYS.find((c) => c.key === filter);
    if (!entry) return classes;
    return classes.filter(
      (c) => c.category === entry.category && (!entry.kind || c.kind === entry.kind),
    );
  }, [classes, filter]);
...
function ClassCard({
  ...
  return (
    <TouchableOpacity
      ...
      onPress={onOpen}
    >
      <Image source={{ uri: cls.image || categoryImage(cls.category, cls.kind) }} style={styles.cardImage} />
      <View style={styles.cardContent}>
        <View style={styles.cardHeaderRow}>
          ...
          >
            <Text style={styles.kindPillText}>{cls.kind === "private" ? "Individuel" : "Groupe"}</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  timePillText: { color: "#fff", fontSize: fontSizes.md, fontWeight: "700" },
  kindPill: {
    paddingHorizontal: 14,
    paddingVertical: 6,
    borderRadius: radius.pill,
  ...
  kindPillText: { color: colors.textPrimary, fontSize: fontSizes.md, fontWeight: "600" },
